 ...
 * sd_journal_stream_fd_with_namespace(). This sink cannot utilize extra fields
 * like TID, CODE_LINE, or CODE_FUNC since sd_journal_send() does not have a
 * namespace-specific counterpart. Journal does interpret kernel-style log level
 * prefixes though, so PRIORITY field is populated correctly.
 */
template <typename Mutex>
class systemd_namespace_sink : public base_sink<Mutex> {
public:
  explicit systemd_namespace_sink(std::string ident, std::string name_space,
    ...
    if (name_space.empty()) {
      ...
      stream_fd = ::sd_journal_stream_fd_with_namespace(
          name_space.c_str(), ident.c_str(), LOG_INFO, 1);
    }
    if (stream_fd < 0) {
      throw_spdlog_ex("Failed opening systemd journal stream to namespace '" +
                          name_space + "'",
                      -stream_fd);
    }
    journal_ = ::fdopen(stream_fd, "w");
    if (journal_ == nullptr) {
      // Capture errno from the failed fdopen() instead of close() (in case that
      // fails too)
    ...
    if (::setvbuf(journal_, nullptr, _IOLBF, 0) != 0) {
      // Capture errno from the failed setvbuf() instead of close() (in case
      // that fails too)
      const int saved_errno = errno;
      ::close(stream_fd);
      throw_spdlog_ex("Failed setting line buffering mode for systemd journal "
                      "stream to namespace '" +
                          name_space + "'",
                      saved_errno);
    }
...
protected:
  ...
  void sink_it_(const details::log_msg &msg) override {
    ...
    // Journal stream is line-oriented; if there's newlines in the payload, send
    // each newline-delimited piece separately as its own message.
    size_t pos = 0;
    while (pos < payload.size()) {
      size_t nl_pos = payload.find('\n', pos);
      size_t end =
          (nl_pos == string_view_t::npos) ? payload.size() : nl_pos + 1;
      string_view_t one_message = payload.substr(pos, end - pos);

      // Write log level prefix
      const auto &prefix = level_prefixes_.at(
          static_cast<level_prefix_array::size_type>(msg.level));
...
inline std::shared_ptr<logger> systemd_namespace_logger_mt(
    const std::string &logger_name, const std::string &ident,
    const std::string &name_space, bool enable_formatting) {
  return Factory::template create<sinks::systemd_namespace_sink_mt>(
      logger_name, ident, name_space, enable_formatting);
}

template <typename Factory = spdlog::synchronous_factory>
inline std::shared_ptr<logger> systemd_namespace_logger_st(
    const std::string &logger_name, const std::string &ident,
    const std::string &name_space, bool enable_formatting) {
