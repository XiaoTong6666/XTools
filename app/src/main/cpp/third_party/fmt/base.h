...
public:
  FMT_CONSTEXPR auto parse(parse_context<Char> &ctx) -> const Char * {
    ...
    if FMT_CONSTEXPR20 (TYPE == type::char_type)
...
public:
  ...
  FMT_CONSTEXPR auto get(int id) const -> format_arg {
    ...
    if (unsigned(id) >= detail::max_packed_args)
      return arg;
    arg.type_ = type(id);
